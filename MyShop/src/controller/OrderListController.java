...
//		order.setFarmCheck(Boolean.parseBoolean(request.getParameter("farmCheck")));
		order.setFarmCheck(true);	// 위의 방법으로 값을 못받아와서 수정시 무조건 true로 바뀌도록 해둠.
		order.setTrackNum(request.getParameter("trackNum"));
		order.setIs_status(request.getParameter("is_status"));
